...
const Recurrences: React.FC<RecurrencesProps> = ({
    templates, categories, onAddTemplate, onEditTemplate, onDeleteTemplate, onApplyTemplates
}) => {
    ...
    const handleOpenNew = () => {
        ...
        setFormData({ type: activeTab, value: '', category_id: '', description: '', dayOfMonth: '5' });
        setIsModalOpen(true);
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.category_id || !formData.value || !formData.dayOfMonth) return;
        ...
        <div className="space-y-6 pb-24 px-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium mt-1">Gerencie seus templates de registros fixos</p>
                </div>
                ...
                {(['Saída', 'Entrada', 'Investimento'] as RecordType[]).map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${
                                isActive ? `bg-${color}-50 text-${color}-600 shadow-sm` : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                            }`}
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTemplates.length > 0 ? (
                            ...
                            <div key={template.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3 group hover:border-blue-200 transition-colors">
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-bold text-gray-900">{template.category_name}</h3>
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center p-12 text-center text-gray-400">
                        ...
                        <p className="text-xs mt-1">Crie um template para lançar rapidamente nos próximos meses.</p>
                    </div>
                )}
            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <select
                                    ...
                                    value={formData.type}
                                    onChange={(e) => setFormData({ ...formData, type: e.target.value as RecordType, category_id: '' })}
                                >
            ...
            {isApplyModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                            <h3 className="text-lg font-extrabold text-gray-900 tracking-tight">
                                Lançar {activeTab === 'Saída' ? 'Despesas' : activeTab === 'Entrada' ? 'Receitas' : 'Investimentos'}
                            </h3>
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div className="p-3 bg-emerald-50 rounded-xl text-xs text-emerald-700 font-medium">
                                Registros já existentes no mês selecionado serão ignorados automaticamente.
                            </div>
                            ...
                            <div className="pt-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">
                                    Templates Selecionados ({selectedTemplateIds.length})
                                </label>
                                    ...
                                    {filteredTemplates.map(t => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-gray-900">{t.category_name}</p>
